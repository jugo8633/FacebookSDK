...
public abstract class FacebookButtonBase extends Button
{
	...
	protected FacebookButtonBase(final Context context, final AttributeSet attrs, int defStyleAttr, int defStyleRes,
			final String analyticsButtonCreatedEventName, final String analyticsButtonTappedEventName)
	{
	...
	/**
	 * Sets the fragment that contains this control. This allows the button to
	 * be embedded inside a Fragment, and will allow the fragment to receive the
	...
	protected void onDraw(Canvas canvas)
	{
		...
		if (centered)
		{
			// if the text is centered, we need to adjust the frame for the
			// titleLabel based on the
			// size of the text in order to keep the text centered in the button
			// without adding
			// extra blank space to the right when unnecessary
			// 1. the text fits centered within the button without colliding
			// with the image
			// (imagePaddingWidth)
			// 2. the text would run into the image, so adjust the insets to
			// effectively left align
			// it (textPaddingWidth)
	...
	private void parseBackgroundAttributes(final Context context, final AttributeSet attrs, final int defStyleAttr,
			final int defStyleRes)
	{
		// TODO, figure out why com_facebook_button_like_background.xml doesn't
		// work in designers
		if (isInEditMode())
		{
			return;
		...
		try
		{
			...
			else
			{
				...
				// 0x7f070000
				setBackgroundColor(Color.argb(127, 07, 00, 00));
			}
		}
		finally
		{
			...
			final int defStyleAttr, final int defStyleRes)
	{
		...
		try
		{
			setCompoundDrawablesWithIntrinsicBounds(a.getResourceId(0, 0), a.getResourceId(1, 0), a.getResourceId(2, 0),
					a.getResourceId(3, 0));
			...
			final int defStyleRes)
	{
		...
		final TypedArray gravityAttrs = context.getTheme().obtainStyledAttributes(attrs, gravityResources, defStyleAttr,
				defStyleRes);
		try
		{
	...
	{
		// set the listener on super so that consumers can set another listener
		// that this will
		// forward to
