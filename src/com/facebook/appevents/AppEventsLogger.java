 ...
 * </p>
 * <ul>
 * <li>Logging predefined and application-defined events to Facebook App
 * Insights with a numeric value to sum across a large number of events, and an
 * optional set of key/value parameters that define "segments" for this event
 * (e.g., 'purchaserStatus' : 'frequent', or 'gamerLevel' : 'intermediate').
 * These events may also be used for ads conversion tracking, optimization, and
 * other ads related targeting in the future.</li>
 * <li>Methods that control the way in which events are flushed out to the
 * Facebook servers.</li>
 * </ul>
 * <p>
 ...
 * out to the Facebook servers in a number of situations:
 * <ul>
 * <li>when an event count threshold is passed (currently 100 logged events).
 * </li>
 ...
 * foreground.</li>
 * </ul>
 * <li>Events will be accumulated when the app is in a disconnected state, and
 * sent when the connection is restored and one of the above 'flush' conditions
 * are met.</li>
 * <li>The AppEventsLogger class is intended to be used from the thread it was
 * created on. Multiple AppEventsLoggers may be created on other threads if
 * desired.</li>
 * <li>The developer can call the setFlushBehavior method to force the flushing
 * of events to only occur on an explicit call to the `flush` method.</li>
 * <li>The developer can turn on console debug output for event logging and
 * flushing to the server by calling
 ...
 * <ul>
 * <li>There is a limit on the number of unique event names an app can use, on
 * the order of 1000.</li>
 * <li>There is a limit to the number of unique parameter names in the provided
 ...
 * <li>Event names and parameter names must be between 2 and 40 characters, and
 * must consist of alphanumeric characters, _, -, or spaces.</li>
 * <li>The length of each parameter value can be no more than on the order of
 * 100 characters.</li>
 * </ul>
 * </p>
 */
public class AppEventsLogger
{
	 ...
	 *            character. Any given app should have no more than 1000
	 ...
	 *            parameter values. You can log on the order of 25 parameters
	 ...
	 *            This should have no more than 24 entries, and keys should be
	...
	private static FlushStatistics buildAndExecuteRequests(FlushReason reason, Set<AccessTokenAppIdPair> keysToFlush)
	{
		...
		for (AccessTokenAppIdPair accessTokenAppId : keysToFlush)
		{
			...
			GraphRequest request = buildRequestForSession(accessTokenAppId, sessionEventsState, limitEventUsage,
					flushResults);
			if (request != null)
			{
	...
	private static GraphRequest buildRequestForSession(final AccessTokenAppIdPair accessTokenAppId,
			...
			synchronized (staticLock)
			{
				if (anonymousAppDeviceGUID == null)
				{
					...
					if (anonymousAppDeviceGUID == null)
					{
						...
						context.getSharedPreferences(APP_EVENT_PREFERENCES, Context.MODE_PRIVATE).edit()
								.putString("anonymousAppDeviceGUID", anonymousAppDeviceGUID).apply();
					}
				}
			}
		}
		...
		private void populateRequest(GraphRequest request, int numSkipped, JSONArray events, boolean limitEventUsage)
		{
			...
			try
			{
				publishParams = AppEventsLoggerUtility.getJSONObjectForGraphAPICall(
						AppEventsLoggerUtility.GraphAPIActivityType.CUSTOM_APP_EVENTS, attributionIdentifiers,
						anonymousAppDeviceGUID, limitEventUsage, applicationContext);
	...
	static class AppEvent implements Serializable
	{
		...
		public AppEvent(String contextName, String eventName, Double valueToSum, Bundle parameters,
		...
		private void validateIdentifier(String identifier) throws FacebookException
		{
			...
			if (identifier == null || identifier.length() == 0 || identifier.length() > MAX_IDENTIFIER_LENGTH)
			{
				...
				throw new FacebookException(String.format(Locale.ROOT,
						"Identifier '%s' must be less than %d characters", identifier, MAX_IDENTIFIER_LENGTH));
			}
				...
				if (hasChanges)
				{
					try
					{
						oos = new ObjectOutputStream(new BufferedOutputStream(
								context.openFileOutput(PERSISTED_SESSION_INFO_FILENAME, Context.MODE_PRIVATE)));
